...
namespace dro_mpc {
    ...
    void CSVWriter::write_header() {
        ofs_ << "seed,plant_seed,predictor_seed,controller_seed,method,scenario,S,eps_wass,sigma,shift_rho,shift_boost,ground_cost,"
            ...
            << "completed_path,termination_reason,total_progress,"
            << "control_effort,constraint_active_count,"
    ...
    RolloutRecord run_experiment_rollout(
        ...
        // Wrapped spline coordinate used for path lookup/controller calls.
        double path_progress =
            ref_path.find_closest_point(ego.position());

        // Unwrapped/net progress used only for deciding whether the route was completed.
        double completion_progress = 0.0;
        ...
        for (int step = 0; step < config.rollout.rollout_steps; ++step) {
            ...
            const Eigen::Vector2d previous_position = ego.position();
            const double previous_path_progress = path_progress;
            ...
            // Update the local spline coordinate used by the controller.
            path_progress =
                ref_path.find_closest_point(
                    ego.position(),
                    previous_path_progress);

            // Measure how far the vehicle actually moved forward along the path.
            const PathPoint previous_path_point =
                ref_path.get_point_at(previous_path_progress);

            const Eigen::Vector2d tangent(
                std::cos(previous_path_point.heading),
                std::sin(previous_path_point.heading));

            const Eigen::Vector2d displacement =
                ego.position() - previous_position;

            const double forward_distance =
                displacement.dot(tangent);

            if (std::isfinite(forward_distance) &&
                forward_distance > 0.0) {
                completion_progress += forward_distance;
            }
            ...
            if (step % 20 == 0 || step == config.rollout.rollout_steps - 1) {
                std::cerr
                    << "[PATH COMPLETION]"
                    << " step=" << step + 1
                    ...
                    << " fraction="
                    << (path_length > 0.0
                            ? completion_progress / path_length
                            : 0.0)
                    << " required_fraction="
                    << config.environment.path_completion_fraction
                    << " termination_enabled="
                    << config.environment.path_completion_termination
                    << " completed="
                    << (path_length > 0.0 &&
                        completion_progress >=
                            config.environment.path_completion_fraction *
