...
int main() {
    ...
    for (int t = 0; t < steps; ++t) {
        ...
        if (ego.v > cfg.mpc.ego.dynamics.max_velocity + 1e-3 || ego.v < cfg.mpc.ego.dynamics.min_velocity - 1e-3) {
            std::printf("  VIOLATION at step %d: v=%.6f (bounds [%.2f, %.2f])\n",
                        t, ego.v, cfg.mpc.ego.dynamics.min_velocity, cfg.mpc.ego.dynamics.max_velocity);
            ++fails;
        }
    }
    ...
    bool respected   = (fails == 0) && (v_max_seen <= cfg.mpc.ego.dynamics.max_velocity + 1e-3)
                                    && (v_min_seen >= cfg.mpc.ego.dynamics.min_velocity - 1e-3);
    (void)tol;
